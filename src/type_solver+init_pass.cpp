...
namespace {
    ...
    void ResolveGroup(const std::vector<TypeVar>& symbols, std::map<TypeVar, std::size_t>* symbolGroup, std::vector<ConstraintGroup>* groups, const ConstraintPass::IDType& newId) {
        ...
        if (constraintGroups.empty()) {
            ...
            groups->at(newGroupId).add(newId);
            ...
            groups->at(firstGroupId).add(newId);
...
auto TypeSolver::SplitToGroups(const typecheck::TypeManager* manager) const -> std::vector<ConstraintGroup> {
    ...
    for (const auto& constraint : manager->constraints) {
        ...
        if (constraint.has_overload()) {
            ...
            for (const auto& func : manager->functions) {
                if (func.id() == constraint.overload().functionid()) {
                    // Merge based on functions
